//! Alternative implementations of the atan2 function to optimize performance.
//!
...
//!
//! This module provides a 3rd degree polynomial approximation that is faster but less
//! accurate (typical error < 0.1 rad)
//!
//! The implementation can be used by enabling the corresponding feature flag:
//! - `atan2_deg3`
//!
//! If no feature is enabled, Rust's default `atan2` implementation is used.
...
mod coeff {
    ...
    fn atan2<F: Float + FloatConst + From<f32>>(y: F, x: F) -> F {
        let abs_y = y.abs();
        let abs_x = x.abs();
    ...
    /// and Horner's method for optimal polynomial evaluation
    fn raw_atan_11<F: Float + From<f32>>(x: F) -> F {
